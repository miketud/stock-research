...
export function InsiderFormsTable({
  ...
  return (
    <section className="mt-8 border-4 border-border-strong bg-surface shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
      <div className="research-panel-header flex flex-wrap items-center justify-between gap-3 border-b-4 border-border-strong px-4 py-3">
        <h3 className="font-black uppercase">Insider ownership</h3>
        <div className="flex flex-wrap items-center justify-end gap-3">
          {activity && (
            <div className="flex flex-wrap gap-x-4 gap-y-1 font-mono text-xs font-bold opacity-75">
          ...
          {details && limit < (details.filings?.length ?? 0) && (
            <button
              ...
              className="border-2 border-border-strong bg-surface px-3 py-1 font-mono text-xs font-black text-text shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all hover:translate-x-0.5 hover:translate-y-0.5 hover:bg-primary hover:text-primary-contrast hover:shadow-none disabled:cursor-wait disabled:opacity-50"
            >
      ...
      {loading ? (
        ...
        (details.filings?.length ?? 0) === 0 ? (
          ...
          <div>
            <div className="flex flex-wrap items-center justify-between gap-3 border-b-2 border-border bg-surface-alt px-4 py-2">
              <p className="font-mono text-[11px] font-black uppercase text-text-muted">
              ...
              <span className="font-mono text-xs font-bold text-text-muted">
                ...
                files shown · {details.transactions.length} transactions · {details.filingsScanned}{' '}
                files read
              </span>
            </div>
            {error && (
              ...
              <table className="w-full table-fixed border-collapse text-left">
                ...
                <tbody>
                  {(details.filings ?? []).slice(0, limit).map((filing, filingIndex) => {
                    ...
                    const wasScanned = details.scannedAccessions?.includes(filing.accessionNumber);
                    const isInitialStatement = /^3(?:\/A)?$/.test(filing.type);
                    const ownerNames =
                      filing.ownerNames && filing.ownerNames.length > 0
                        ? filing.ownerNames
                        : [...new Set(matchedTransactions.map((transaction) => transaction.ownerName))];
                        ...
                        >
                          ...
                          <td className="px-3 py-2">
                            {matchedTransactions.length > 0 ? (
                              ...
                              <span className="font-mono text-xs text-text-muted">{detailSummary}</span>
                            )}
                          </td>
                                    ...
                                    >
                                      ...
                                      <div className="min-w-0">
                                        ...
                                        <dd className="truncate text-[9px] text-text-muted">
                                          {transaction.transactionCode
                                            ? transactionCodeLabels[transaction.transactionCode] ||
                                              'Other'
                                            : 'Uncoded'}
